...
Primeiramente, começaremos por simplificar as funções q, r e c de modo a que tenhamos
expressões pointfree destas mesmas de modo a facilitar encontrar o gene do catamorfismo de cada uma e,
consequentemente podermos aplicar a lei de Fokkinga.\\
Começaremos por q que pode ser representada pela seguinte expressão:
\begin{eqnarray*}
\start
%
         |lcbr(
          ...
          (q d) . succ = (c d) == 0 -> succ . (q d), (q d)
     )|
%
\just\equiv{ Eq-+ }
%
     |(either ((q d) . const 0) ((q d) . succ)) =
       either (const 0)  ((c d) == 0 -> succ . (q d), (q d))|
%
\just\equiv{ Fusão-+ }
%
     |(q d) . (either (const 0) (succ)) =
       either (const 0) ((c d) == 0 -> succ . (q d), (q d))|
%
\just\equiv{ Def do in nos naturais }
%
     |(q d) . in = (either (const 0) ((c d) == 0 -> succ . (q d), (q d)))|
%
\just\equiv{ 2 Lei da Fusao condicional }
%
     |(q d) . in = (either (const 0) (((c d) == 0 -> succ, id) . p1 . (aux d)))|
%
\just\equiv{ Absorção-+ }
% 
     |(q d) . in = (either (const 0) (((c d) == 0 -> succ, id) . p1)) . (id + (aux d))|
%
\just\equiv{ Definição do Functor dos Naturais}
%
     |(q d) . in = (either (const 0) (((c d) == 0 -> succ, id) . p1))  . F (aux d)|
%
\end{eqnarray*}
Assim, temos q simplificado e pronto para ser utilizado na Regra de Fokkinga, trataremos então de 
simplificar também as outras expressões:
\begin{eqnarray*}
\start
%
         |lcbr(
          ...
          (r d) . succ = (c d) == 0 ==> const 0, succ . (r d)
     )|
%
\just\equiv{ Eq-+ }
%
...
\just\equiv{ Fusão-+ }
%
     |(r d) . either (const 0) (succ) = either (const 0) ((c d) == 0 ==> const 0, succ . (r d))|
%
\just\equiv{ Def de in em Nat}
%
     |(r d) . in = either (const 0) ((c d) == 0 ==> const 0, succ . (r d))|
%
\just\equiv{ Natural-Const }
%
     |(r d) . in = either (const 0) ((c d) == 0 ==> const 0 . (r d) , succ . (r d))|
%
\just\equiv{ 2 lei da fusao cond }
%
     |(r d) . in = either (const 0) (((c d) == 0 ==> const 0, succ) . p2 . p1 . aux d)|
%
\just\equiv{ absorcao-+ }
%
     |(r d) . in = either (const 0) (((c d) == 0 ==> const 0, succ) . p2 .p1) . (id + aux d)|
%
\just\equiv{ Def do Functor dos Naturais}
%
     |(r d) . in = either (const 0) (((c d) == 0 ==> const 0, succ) . p2 . p1) . (F aux d)|
%
\end{eqnarray*}
E finalmente para c, onde subb é definido da seguinte forma:
\begin{code}
subb n = n - 1
\end{code}
(Tal que subb . succ = id)
\begin{eqnarray*}
\start
%
        |lcbr(
...
\just\equiv{ Fusão-+ }
%
     |(c d) . either (const 0) (succ) = either (const d) ((c d) ==> const d, subb . (c d))|
%
\just\equiv{ Def de in em Nat }
%
     |(c d) . in = either (const d) ((c d) ==> const d, subb . (c d))|
%
\just\equiv{ Natural-Const }
%
     |(c d) . in = either (const d) ((c d) == 0 ==> const d . (c d) , subb . (c d))|
%
\just\equiv{ 2 lei da fusão condicional seguida de absorção\-+ }
%
     |(c d) . in = either (const d) (((c d) == 0 -> const d, subb) . pi2 . pi2) . (id + aux d)|
%
\just\equiv{ Def do Functor dos Naturais}
%
     |(c d) . in = either (const d) (((c d) == 0 ==> const d, subb) . pi2 . pi2) . F aux d|
%
\end{eqnarray*}
Assim estamos em condicões para aplicar a lei de Fokkinga:

\begin{eqnarray*}
\start
%
          |lcbr(
          (q d) . in = (geneQ d) . F aux d
     )(
          (r d) . in = (geneR d) . F aux d
     )(
          (c d) . in = (geneC d) . F aux d
     )|
\just\equiv{ Lei de Fokkinga }
%
     |aux d = cataNat (split (geneQ d) (split (geneR d) (geneC d)))|
%
\just\equiv{ Def dos genes }
%
     |(aux d) = cataNat (split (either (const 0) (i2 . qTMP))
                        (split (either (const 0) (i2 . rTMP))
                               (either (const d) (i2 . cTMP))))|
%
\just\equiv{ Lei da Troca no innermost split}
%
     |(aux d) = cataNat (split  (either (const 0) (i2 . qTMP)
                        (either (split (const 0) (const d))
                                (split (i2 . rTMP)
                                       (i2 . cTMP))|
%
\just\equiv{ Lei da Troca }
%
     |(aux d) = cataNat (either (split (const 0) (split (const 0) (const d)))
                        (split  (i2 . qTMP)
                                (split (i2 . rTMP)
                                       (i2 . cTMP)))|
%
...
%
    |(aux d) = cataNat (either (split (const 0) (split (const 0) (const d)))
                       (split  (i2 . qTMP)
                               (split (i2 . rTMP)
                                      (i2 . cTMP))))|
\just\equiv{ Def de for e definição de loop d, igualdade extensional }
%
     |(aux d) = (loop d)|
\qed
\end{eqnarray*}
...
\end{eqnarray*}
\\
Assim, temos que da forma como simplificamos (aux d) temos exatamente
\begin{eqnarray*}
\start
   |cataNat (either (split (const 0) (split (const 0) (const d))) (g d))|
\end{eqnarray*}
Logo, aux d e loop d são a mesma função.
